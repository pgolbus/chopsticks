...
from service.chopsticks.DAOs.sqlite_dao import SQLiteDAO
...
def test_get_player_hands(sqlite_dao, mocked_conn):
    ...
    player = sqlite_dao.get_player_hands(1)
...
def test_set_player_hands(sqlite_dao, mocked_conn):
    # Extract the mocked connection and cursor from the fixture
    conn_mock, cursor_mock = mocked_conn

    # Call the method to set player hands
    sqlite_dao.set_player_hands(1, 4, 5)

    # Check if the correct SQL update command was executed
    cursor_mock.execute.assert_called_once_with(
        'UPDATE players SET left_hand = ?, right_hand = ? WHERE player_id = ?',
        (4, 5, 1)
    )

    # Ensure the transaction was committed to the database
